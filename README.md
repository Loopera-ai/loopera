...
<p align="center">
  ...
  <a href="./assets/demo/loopera_demo.mp4">
...
```mermaid
flowchart TB
    subgraph P1["01 · 发现与解释"]
      direction LR
      A["1 发现异常"] --> B["2 比较机制"] --> C["3 选择可验证解释"]
    end
    subgraph P2["02 · 构建与审查"]
      direction LR
      D["4 形成研究契约"] --> E["5 验证数据与实现"] --> F["6 盲审经济逻辑"]
    end
    subgraph P3["03 · 评价与学习"]
      direction LR
      G["7 评价表现与增量"] --> H["8 比较候选家族"] --> I["9 写入研究记忆"]
    end
    C --> D
    F --> G
    I -. "反哺下一轮" .-> A

    classDef discovery fill:#E8F7FF,stroke:#168CFF,color:#07142E,stroke-width:1.5px;
    classDef validation fill:#F1ECFF,stroke:#7C3AED,color:#07142E,stroke-width:1.5px;
    classDef learning fill:#E8FFF7,stroke:#0F9F8F,color:#07142E,stroke-width:1.5px;
    class A,B,C discovery;
    class D,E,F validation;
    class G,H,I learning;
```
...
```mermaid
flowchart LR
    A["147 次候选尝试"] --> B["130 个有效候选"]
    A --> X["17 个未形成有效候选<br/>执行或生成问题"]
    B --> C["125 个明确停止"]
    B --> D["5 个进入研究库"]

    classDef total fill:#E8F7FF,stroke:#168CFF,color:#07142E,stroke-width:1.5px;
    classDef stopped fill:#FFF1F2,stroke:#E5484D,color:#07142E,stroke-width:1.5px;
    classDef accepted fill:#E8FFF7,stroke:#0F9F8F,color:#07142E,stroke-width:1.5px;
    class A,B total;
    class X,C stopped;
    class D accepted;
```
